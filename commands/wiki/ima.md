...
9. If roadmap planning changes are **not** implied by the user's natural-language intent, leave roadmap files unchanged. In the report, list proposed roadmap edits under "Suggested roadmap updates".
10. If roadmap planning changes **are** implied by the user's natural-language intent:
...
11. Update `agents/index.md` with any new research/docs pages.
12. Append one line to `agents/roadmap/log.md`:

   ```markdown
   | {today} | wiki-ima | {slug} analyzed; roadmap update: {yes/no} |
   ```

13. If `agents/graph/ontology.yaml` exists, mention that the derived graph is now stale and suggest running `/wur:wiki:graph extract`.
14. Report: raw source stored, pages created/updated, roadmap files changed or suggested, specialist roles used, contradictions found, open questions, and next safe command.
