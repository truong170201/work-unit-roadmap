    ...
    def test_init_contract_installs_and_verifies_enforcement(self) -> None:
        ...
        self.assert_contains_all(
            ...
            [
                ...
                "If a worktree is used, exclude `agents/` and `contracts/`",
    ...
    def test_start_contract_creates_one_file_execution_contract(self) -> None:
        ...
        self.assert_contains_all(
            ...
            [
                ...
                "executor may update only Status and Commit cells for touched WU rows",
                ...
                "Reports outside `contracts/` or outside the active phase contract are invalid",
                "Commit provenance must match the WU context",
                ...
                "must not edit the phase contract header, Goal, Success Criteria, Scope, Dependencies, Verification, Allowed Read References, or any other contract file",
                "may suggest only `active`, `ready-for-review`, `blocked`, or `deferred`",
                "Do not create a worktree just because a contract exists",
                "If there is no implementation task, do not create a worktree",
                "If implementation requires editing project files, create or reuse a sparse worktree",
    ...
    def test_done_contract_gates_closeout_and_preserves_history(self) -> None:
        ...
        self.assert_contains_all(
            ...
            [
                ...
                "Only reports appended inside the active phase contract are valid closeout evidence",
                "Reports outside `contracts/` or outside the active phase contract are invalid",
                "commit provenance matches the WU context",
                "quotes the exact current-client authorization",
    ...
    def test_upgrade_contract_is_migration_not_reset(self) -> None:
        ...
        self.assert_contains_all(
            ...
            [
                ...
                "Canonicalize `agents/SCHEMA.md`, not just append missing sections",
                "Replace stale canonical schema lines when they conflict with the schema-1 template",
                "Legacy Phase Fix entries may remain only as legacy-readable compatibility",
                "They must not describe Phase Fix as the current/new-work flow",
                "## Contract Boundary",
                "`contracts/rule.md` stores shared execution rules outside `agents/`",
                "`contracts/PHASE_{n}_CONTRACT.md` stores phase task brief, Allowed Read References, and executor report ledger",
                "If a worktree is used, exclude `agents/` and `contracts/`",
    ...
    def test_contract_model_replaces_worktree_sync_and_codex_delegation(self) -> None:
        ...
        self.assert_contains_all(
            ...
            [
                ...
                "Executors may update only Status and Commit cells for touched WU rows",
                "Executor-owned Pending Work statuses may only become `active`, `ready-for-review`, `blocked`, or `deferred`",
                ...
                "Do not use a docs, contract-cleanup, or planning commit as implementation evidence",
                "For every touched WU, update its Pending Work Status and Commit cells before appending the report",
                "Executor reports must include lifecycle evidence for every touched WU",
                "Executor status suggestions without explicit client approval stop at `active`, `ready-for-review`, `blocked`, or `deferred`",
                "Do not create `contracts/outbox/` or `contracts/inbox/`",
                "Do not create Phase Fix ledgers for new work",
                "Sparse worktree guidance",
                "executor first reads the active contract",
                "If there is no implementation task, it reports blocked/no-op/clarification-needed without creating a worktree",
                "If implementation requires editing project files, it creates or reuses a sparse worktree",
            ],
        )
        self.assert_contains_all(
            readme,
            [
                ...
                "may update only touched WU Status/Commit cells",
                "appends evidence under `## Execution Rounds And Reports`",
                "Reports outside `contracts/` or outside the active phase contract are invalid",
                "Commit provenance must match the WU context",
                "For every touched WU, the executor updates Pending Work Status/Commit cells before appending the report",
                "first checks whether the contract contains executable project work",
                "If not, it reports blocked/no-op/clarification-needed without creating a worktree",
                "If implementation requires editing project files, it creates or reuses a sparse worktree",
                "including WU lifecycle evidence and matching Pending Work table updates",
                "accepted/done requires explicit current-client approval for exact WU IDs",
                "`agents/reports/` stores durable WUR reports and summaries",
                "research synthesis, verification, audits, receive summaries, completion reports, archives",
                "Allowed Read References",
        ...
        self.assert_contains_all(
            ...
            [
                ...
                "Executors may update only Status and Commit cells for touched WU rows",
                "matching Pending Work table updates",
                "Only reports appended inside the active phase contract are valid WUR receive evidence",
                "Commit provenance must match the WU context",
                "For every touched WU, update its Pending Work Status and Commit cells before appending the report",
                "Reports must include WU lifecycle evidence",
                "WUR applies roadmap state after receive",
